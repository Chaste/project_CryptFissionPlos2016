...
#ifndef TESTEPITHELIALLAYERWITHTARGETRATIO_HPP_
#define TESTEPITHELIALLAYERWITHTARGETRATIO_HPP_
...
class TestEpithelialLayerWithTargetRatio : public AbstractCellBasedTestSuite
{
public:
	void TestTissueRingWithTargetRatio() throw(Exception)
	{
		...
		double target_proportions [3] = {0.3, 0.65, 0.8};

		for (double pstiff = 0.0; pstiff < 3.0; pstiff ++)
		{
			for (unsigned tratio = 0; tratio < 3; tratio ++)
			{

				//Set the Paneth cell spring stiffness multiplier and the target proportion of
				//stem cells to Paneth cells.
				double paneth_stiffness_multiplier = 3.0 + 0.75*pstiff;
				double target_proportion = target_proportions[tratio];
				...
				//Set Paneth cells to be 1.3 times as adhesive as stem cells, by altering the damping constant.
				//Thus \eta_P = 1.3\eta_S (Nathke group, personal correspondence).
				double normal_damping_constant = cell_population.GetDampingConstantNormal();
				cell_population.SetDampingConstantMutant(1.3*normal_damping_constant);
				...
				run << 100*index;
				std::string output_directory = "EpithelialLayerWithTargetRatio/" + out.str() + "/"
						+ run.str() + "/" + params.str();
				...
				for (unsigned k=0; k<angles_and_cell_indices.size(); k++)
				{
					...
					if(!cell->GetMutationState()->IsType<PanethCellMutationState>() ) //Stem cell if 0, Paneth cell if 1 (confusing, I know, but it's too late to change the mistake now after running all the simulations)
					{
						is_stem_cell = 0;
					}
...
#endif /* TESTEPITHELIALLAYERWITHTARGETRATIO_HPP_ */
